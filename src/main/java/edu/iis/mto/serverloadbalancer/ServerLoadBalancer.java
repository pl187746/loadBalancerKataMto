...
public class ServerLoadBalancer {
	...
	private void addToCapableLessLoadedServer(Server[] servers, Vm vm) {
		List<Server> serversWithEnoughCapacity = findServersWithEnoughCpacity(servers, vm);			
		addToLessLoadedServer(vm, serversWithEnoughCapacity);
	}

	private void addToLessLoadedServer(Vm vm, List<Server> serversWithEnoughCapacity) {
		Server lessLoaded = extractLessLoadedServer(serversWithEnoughCapacity);
		if(lessLoaded != null)
			lessLoaded.addVm(vm);
	}

	private List<Server> findServersWithEnoughCpacity(Server[] servers, Vm vm) {
